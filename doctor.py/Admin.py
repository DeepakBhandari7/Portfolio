...
class Admin:
    ...
    # patient from same family
    
    def group_patients(self,patients):
        
        same_family = {}  # Dictionary to hold patients grouped by family name

        for patient in patients:
            surname = patient.get_surname()  # Using the get_surname method to get the surname
            if surname not in same_family:
                same_family[surname] = [patient]
            else:
                same_family[surname].append(patient)
    ...
    def doctor_management(self, doctors):
        ...
        if op == '1':
            ...
            first_name,doctor_surname,doctor_speciality=self.get_doctor_details()

            # check if the name is already registered
            name_exists = False
            for doctor in doctors:
                if first_name == doctor.get_first_name() and doctor_surname == doctor.get_surname():
                    print('Name already exists.')
                    name_exists=True
                    break

            if(name_exists==False):
                doctors.append(Doctor(first_name,doctor_surname,doctor_speciality))
